...
export default function RiskPage() {
  ...
  async function run() {
    ...
    setResult(null);
    try {
      const out = await postJSON("/api/risk", { session });
      setResult(out);
    } catch (e: any) {
  ...
  return (
    <div style={{ background: "white", border: "1px solid #eee", borderRadius: 18, padding: 18 }}>
      <h1 style={{ marginTop: 0 }}>Risk</h1>
      <p style={{ marginTop: -6, opacity: 0.7 }}>Baseline stress score using cashflow + debt + savings buffer.</p>
