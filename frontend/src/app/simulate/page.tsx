"use client";

import { useState } from "react";
import { useSession } from "@/lib/useSession";
import { postJSON } from "@/lib/api";

export default function SimulatePage() {
  const { session, hydrated } = useSession();
  const [months, setMonths] = useState(12);
  const [runs, setRuns] = useState(500);
  const [result, setResult] = useState<any>(null);
  const [err, setErr] = useState<string | null>(null);

  if (!hydrated) return <div>Loading…</div>;

  async function run() {
    setErr(null);
    setResult(null);
    try {
      const out = await postJSON("/api/simulate", { session, months, runs });
      setResult(out);
    } catch (e: any) {
      setErr(String(e.message ?? e));
    }
  }

  return (
    <div style={{ background: "white", border: "1px solid #eee", borderRadius: 18, padding: 18 }}>
      <h1 style={{ marginTop: 0 }}>Monte Carlo</h1>
      <p style={{ marginTop: -6, opacity: 0.7 }}>Runs shock simulations (income drops, expense spikes) to estimate distress probability.</p>

      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <label>Months <input type="number" value={months} onChange={(e) => setMonths(Number(e.target.value))} /></label>
        <label>Runs <input type="number" value={runs} onChange={(e) => setRuns(Number(e.target.value))} /></label>
        <button onClick={run} style={{ padding: "10px 14px", borderRadius: 12, border: "1px solid #ddd", background: "#111", color: "white" }}>
          Run Simulation
        </button>
      </div>

      {err && <p style={{ color: "crimson" }}>{err}</p>}
      {result && <pre style={{ marginTop: 12, background: "#fafafa", padding: 12, borderRadius: 12 }}>{JSON.stringify(result, null, 2)}</pre>}
    </div>
  );
}